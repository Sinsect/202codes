...
*/
public class AssignmentThreeUpdate1 {
    public static void main(String[] args) {
        double increment = 0.25;
        int runCount = 5;
	 	double arrivalRate = increment;
	 	double serviceRate = increment;
		double checkoutRate = arrivalRate / 2;
		int stationCount = 5;
		int checkoutCount = 1;
        double simTime = 1000;
		...
		for (int i = 0; i < runCount; i++) {
			SimulationTest simmie = new SimulationTest(arrivalRate, serviceRate, checkoutRate, stationCount, checkoutCount);
			simmie.run(simTime);
