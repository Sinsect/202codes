...
public class SimulationTest {
    ...
    //Construct
	public SimulationTest(double arrivalRate, double serviceRate, double checkoutRate, int stationCount, int checkoutCount) {
		this.arrivalRate = arrivalRate;
		this.serviceRate = serviceRate;
		this.currentTime = 0;
		this.arrival = new ArrivalProcess(arrivalRate);
		this.completedJobs = new Queue();
        //Set queue size as single entry + station + checkout
        this.queues = new SingleServerQueue[1 + stationCount + checkoutCount];
        checkoutIndex = queues.length - checkoutCount;
        // Initialize queues
        for (int i = 0; i < queues.length; i++) {
            queues[i] = new SingleServerQueue(i < checkoutIndex ? serviceRate : checkoutRate);
        }
    ...
    private void routeToNextQueue(Job completedJob, int currentIndex) {
        ...
        if (currentIndex == 0) {
            nextQueueIndex = (int)Math.round(1 + Math.random() * (queues.length - (1 + queues.length - checkoutIndex)));
        }
        else { 
            nextQueueIndex = (int)Math.round(1 + Math.random() * (queues.length - 1));
        }
        if (nextQueueIndex != queues.length) {
